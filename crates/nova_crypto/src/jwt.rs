...
impl TokenBehavior for AccessToken {
    ...
    fn token_type() -> String { "access".to_string() }
}
...
impl TokenBehavior for RefreshToken {
    ...
    fn token_type() -> String { "refresh".to_string() }
}
...
impl Jwt {
    ...
    pub fn verify(&self, token: &str, token_type: &str) -> bool {
        let decoded = self.decode_token(token);
        if decoded.is_none() {
            error!("Failed to verify jwt token: failed to decode token");
            return false;
        }
        self.verify_decoded(&decoded.unwrap(), token_type)
    }

    pub fn verify_decoded(&self, token: &TokenData<Claims>, token_type: &str) -> bool {
        let claims = &token.claims;

        if claims.token_type != token_type {
            error!("Failed to verify jwt token: token type mismatch. Expected: {}, got: {}", token_type, claims.token_type);
            return false;
        }

        debug!("Successfully verified {token_type} jwt token for subject: {}", claims.sub);
        true
    }

    pub fn decode_token(&self, token: &str) -> Option<TokenData<Claims>> {
        if token.is_empty() {
            error!("Failed to decode jwt token: token is empty");
            return None;
        }
...
mod tests {
    ...
    fn test_verify_valid_token() {
        ...
        assert!(jwt.verify(&tokens.access, "access"));
        assert!(jwt.verify(&tokens.refresh, "refresh"));
    }
    ...
    fn test_verify_invalid_token() {
        let jwt = make_jwt();
        assert!(jwt.is_some());
        assert!(!jwt.unwrap().verify("invalid_token", "access"));
    }
    ...
    fn test_tampered_token_fails() {
        ...
        assert!(!jwt.verify(&tampered, "access"));
    }
}
